...
add_users () {
    ...
    conf-utils setvar -y -s "$USERS_SHARENAME" -n "root preexec" -a "/usr/bin/create_user_share.sh %U" -f "pretty" "$SMBCONF"
}
add_timemachine () {
    ...
    conf-utils setvar -y -q -s "$TIMEMACHINE_SHARENAME" -n "delete veto files" -a "yes" -f "pretty" "$SMBCONF"
